...
void loop() {
  ...
  if (((WiFi.status() != WL_CONNECTED) || (!success)) && (currentMillis - previousMillis >=interval)) {
    if(!success){
    ...
    number_of_reconnects=(number_of_reconnects+1)%2;
    if (number_of_reconnects==0)
      {Serial.println("Rebooting after 5 connection failures.");
        ESP.restart();}
  }
